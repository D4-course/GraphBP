...
class Runner():
    ...
    def _train_epoch(self, loader):
        """
        Function to train 1 epoch for Runner
        """
        self.model.train()
        ...
        for iter_num, data_batch in enumerate(loader):
            ...
            if data_batch['atom_type'].size(0) > 600000:
                ...
                continue
            node_out, focus_score, contact_score, dist_out, angle_out, torsion_out = self.model(
                data_batch)
            ...
            ll_torsion = torch.mean(
                1 / 2 * (torsion_out[0] ** 2) - torsion_out[1])
            ...
            if iter_num % self.conf['verbose'] == 0:
                print(
                    'Training iteration {} | loss node {:.6f} dist {:.6f} angle {:.6f} torsion {:.6f} focus {:.6f} contact {:.6f} duration {:.6f}'.format(
                        iter_num,
                        ll_node.to('cpu').item(),
                        ll_dist.to('cpu').item(),
                        ll_angle.to('cpu').item(),
                        ll_torsion.to('cpu').item(),
                        focus_ce.to('cpu').item(),
                        contact_ce.to('cpu').item(),
                        duration))

        iter_num += 1
        iter_num -= skip_batch_num
        return total_ll_node / iter_num, total_ll_dist / iter_num, total_ll_angle / iter_num, total_ll_torsion / \
            iter_num, total_focus_ce / iter_num, total_contact_ce / iter_num, skip_batch_num

    def train(self, binding_site_range):
        """
        Train function for Runner class
        """
        ...
        for epoch in range(epochs):
            avg_ll_node, avg_ll_dist, avg_ll_angle, avg_ll_torsion, avg_focus_ce, avg_contact_ce, skip_batch_num = self._train_epoch(
                loader)
            print('=============================================')
            print(
                'Training | Average loss node {:.6f} dist {:.6f} angle {:.6f} torsion {:.6f} focus {:.6f} contact {:.6f}'.format(
                    avg_ll_node,
                    avg_ll_dist,
                    avg_ll_angle,
                    avg_ll_torsion,
            ...
            if self.out_path is not None:
                ...
                file_obj.write(
                    'Training | Average loss node {:.6f} dist {:.6f} angle {:.6f} torsion {:.6f} focus {:.6f} contact {:.6f}\n'.format(
                        avg_ll_node,
                        avg_ll_dist,
                        avg_ll_angle,
                        avg_ll_torsion,
    ...
    def generate(
            self, num_gen, temperature=[1.0, 1.0, 1.0, 1.0],
            min_atoms=2, max_atoms=35, focus_th=0.5, contact_th=0.5,
            add_final=False, contact_prob=False,
            data_root='./data/crossdock2020/',
            data_file='./data/crossdock2020/types/mini_test.types',
            atomic_num_to_type=atomic_num_to_type,
            atomic_element_to_type=atomic_element_to_type,
            known_binding_site=False, binding_site_range=15.0):
        """
        Function to generate output for Runner class
        """
        data_cols = [
            'low_rmsd',
            'true_aff',
        ...
        for index in range(len(data_lines)):
            ...
            with warnings.catch_warnings():
                ...
                rec_structure = pdb_parser.get_structure(
                    '', os.path.join(data_root, rec_src))

            rec_atom_type = [
                atomic_element_to_type[atom.element]
            ...
            if known_binding_site:
                try:
                    ...
                    lig_position = np.array(
                        [[float(x) for x in line.split()[: 3]] for line in
                        lig_pos],
                        dtype=np.float32)
                    ...
                    print(f"SDF file for ligand \"{sdf_file}\" not found")

            num_remain = num_gen
            one_time_gen = self.conf['chunk_size']
            type_to_atomic_number_dict = {
                atomic_num_to_type[k]: k for k in atomic_num_to_type}
            type_to_atomic_number = np.zeros(
                [max(type_to_atomic_number_dict.keys()) + 1], dtype=int)
            ...
            while num_remain > 0:
                if num_remain > one_time_gen:
                    mols = self.model.generate(
                        type_to_atomic_number, rec_atom_type, rec_position,
                        one_time_gen, temperature, min_atoms, max_atoms,
                        focus_th, contact_th, add_final, contact_prob)
                else:
                    mols = self.model.generate(
                        type_to_atomic_number, rec_atom_type, rec_position,
                        num_remain, temperature, min_atoms, max_atoms,
                        focus_th, contact_th, add_final, contact_prob)

                for num_atom in mols:
                    if num_atom not in mol_dicts.keys():
                ...
                print(
                    '{} molecules are generated!'.format(
                        num_gen - num_remain))
