...
from main_gen import molecularDictGen
...
@app.post("/pretrained")
def dummy():
	shutil.make_archive('output', 'zip', "/d4/outputs"))
    return FileResponse(path='output', filename='output.zip')
	

@app.post("/predict")
def execute(file: UploadFile):

    with open(DATA_ROOT + '/types/mini_types.types', 'w') as f:
        f.write(file.file.readline())
    ...
    try:
        shutil.make_archive('output', 'zip', os.PATH.join(PATH, 'gen_mols_epoch_' + str(EPOCH)))
        return FileResponse(path='output', filename='output.zip')
    except BaseException:
        pass
