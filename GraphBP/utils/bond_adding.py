"""
Code adapted from
https://github.com/luost26/3D-Generative-SBDD/tree/6f9c7d92784e58474b9c22a74c8113f0344ca795
and https://github.com/mattragoza/liGAN
"""
import numpy as np
...
class MolReconsError(Exception):
    """
    Class to calculate molecular reconstruction error
    """
    pass
...
class BondAdder():
    ...
    def fixup(self, mol):
        ...
        for atom in ob.OBMolAtomIter(mol):
            ...
            if (atom.GetAtomicNum() in (7, 8)
                    ) and atom.IsInRing():     # Nitrogen, Oxygen
    ...
    def connect_the_dots(self, mol, atoms):
        ...
        for (i, a) in enumerate(atoms):
            ...
            # since we want the molecule to be valid for both (rdkit is usually
            # lower)
        ...
        def get_bond_info(biter):
            ...
            for bond in bonds:
                ...
                a1 = bond.GetBeginAtom()
                a2 = bond.GetEndAtom()
                ideal = ob.GetCovalentRad(
                    a1.GetAtomicNum()) + ob.GetCovalentRad(a2.GetAtomicNum())
                stretch = bdist - ideal
                binfo.append((stretch, bdist, bond))
            # most stretched bonds first
        ...
        hypers = sorted([(atom_maxb[a.GetIdx()], a.GetExplicitValence(
        ) - atom_maxb[a.GetIdx()], a) for a in atoms], key=lambda aa: (aa[0], -aa[1]))
        ...
        for stretch, bdist, bond in binfo:
            ...
            # also remove tight angles, because that is what ConnectTheDots
            # does
            if stretch > self.max_bond_stretch or self.forms_small_angle(
                    a1, a2) or self.forms_small_angle(a2, a1):
                # don't fragment the molecule
                if not self.reachable(a1, a2):
                    continue
                mol.DeleteBond(bond)
        ...
        if len(mol.Separate()) > 1:
            sep_mols = sorted(
                [sep_mol for sep_mol in mol.Separate()],
                key=lambda x: x.NumAtoms())
    ...
    def convert_ob_mol_to_rd_mol(self, ob_mol):
        ...
        for (d, bond) in nonsingles:
            ...
            if self.calc_valence(a1) > pt.GetDefaultValence(
                    a1.GetAtomicNum()) or self.calc_valence(a2) > pt.GetDefaultValence(
                    a2.GetAtomicNum()):
        ...
        positions = rd_mol.GetConformer().GetPositions()
        center = np.mean(
            positions[np.all(np.isfinite(positions), axis=1)], axis=0)
        for atom in rd_mol.GetAtoms():
            i = atom.GetIdx()
            pos = positions[i]
            if not np.all(np.isfinite(pos)):
                # hydrogens on C fragment get set to nan (shouldn't, but they
                # do)
    ...
    def postprocess_rd_mol_1(self, rdmol):
        """
        Function to post-process the rd molecule 1
        """
            ...
            if num_radical > 0:
                for j in nbh_list[idx]:
                    ...
                    if nb_radical > 0:
                        ...
                        bond.SetBondType(self.UPGRADE_BOND_ORDER
                                         [bond.GetBondType()])
    ...
    def postprocess_rd_mol_2(self, rdmol):
        """
        Function to post-process the rd molecule 2
        """
        ...
        for i, ring_a in enumerate(rings):
            if len(ring_a) == 3:
                ...
                if 'O' in atom_by_symb and len(atom_by_symb['O']) == 2:
                    ...
                    rdmol_edit.GetAtomWithIdx(
                        atom_by_symb['O'][0]).SetNumExplicitHs(
                        rdmol_edit.GetAtomWithIdx(
                            atom_by_symb['O'][0]).GetNumExplicitHs() + 1)
                    rdmol_edit.GetAtomWithIdx(
                        atom_by_symb['O'][1]).SetNumExplicitHs(
                        rdmol_edit.GetAtomWithIdx(
                            atom_by_symb['O'][1]).GetNumExplicitHs() + 1)
