"""
Code to get Dataset from scratch
"""
import numpy as np
import torch
...
def collate_mols(mol_dicts):
    ...
    for key in [
        ...
        'new_torsion',
            'cannot_focus']:
        data_batch[key] = torch.cat([mol_dict[key]
                                    for mol_dict in mol_dicts], dim=0)

    num_steps_list = torch.tensor([0] +
                                  [len(mol_dicts[i]['new_atom_type'])
                                   for i in range(len(mol_dicts) - 1)])
    batch_idx_offsets = torch.cumsum(num_steps_list, dim=0)
    repeats = torch.tensor([len(mol_dict['batch']) for mol_dict in mol_dicts])
    batch_idx_repeated_offsets = torch.repeat_interleave(
        batch_idx_offsets, repeats)
    batch_offseted = torch.cat(
        [mol_dict['batch'] for mol_dict in mol_dicts],
        dim=0) + batch_idx_repeated_offsets
    data_batch['batch'] = batch_offseted

    num_atoms_list = torch.tensor([0] +
                                  [len(mol_dicts[i]['atom_type'])
                                   for i in range(len(mol_dicts) - 1)])
    ...
    for key in ['focus', 'c1_focus', 'c2_c1_focus', 'contact_y_or_n']:
        ...
        atom_idx_repeated_offsets = torch.repeat_interleave(
            atom_idx_offsets, repeats)
        if key == 'contact_y_or_n':
            atom_offseted = torch.cat(
                [mol_dict[key] for mol_dict in mol_dicts],
                dim=0) + atom_idx_repeated_offsets
        else:
            atom_offseted = torch.cat(
                [mol_dict[key] for mol_dict in mol_dicts], dim=0) + atom_idx_repeated_offsets[:, None]
...
class CrossDocked2020_SBDD(Dataset):
    """
    Class for CrossDocked2020 Dataset for Structure Based Drug Discovery
    """
    def __init__(
            self, data_root='./data/crossdock2020',
            data_file='./data/crossdock2020/it2_tt_0_lowrmsd_mols_train0_fixed.types',
    ...
    def read_rec_mol(self, mol_src):
        ...
        with warnings.catch_warnings():
            ...
            structure = self.pdb_parser.get_structure(
                '', os.path.join(self.data_root, mol_src))
        return structure
    ...
    def __getitem__(self, index):
        ...
        # Read .sdf file instead of .sdf.gz file; Why?
        # (https://github.com/rdkit/rdkit/issues/1938)
        lig_supplier = self.get_lig_mol(example.lig_src.rsplit('.', 1)[0])

        rec_atom_type = [
            self.atomic_element_to_type[atom.element]
            for atom in rec_structure.get_atoms() if atom.element != 'H']
        rec_position = np.stack([atom.coord
...
#         lig_mol = lig_supplier[0]
        ...
        lig_position = np.array(
            [[float(x) for x in line.split()[: 3]] for line in lig_pos],
            dtype=np.float32)
        lig_atom_type = np.array(
            [self.atomic_num_to_type[atom.GetAtomicNum()]
        ...
        lig_atom_bond_valency = torch.tensor(
            np.sum(lig_con_mat, axis=1))  # [lig_n_atoms]
        lig_con_mat = torch.tensor(lig_con_mat)  # [lig_n_atoms, lig_n_atoms]
        del lig_supplier

        # Get binding site
        lig_center = torch.mean(lig_position, dim=0)
        rec_atom_dist_to_lig_center = torch.sqrt(
            torch.sum(torch.square(rec_position - lig_center), dim=-1))
        selected_mask = rec_atom_dist_to_lig_center <= self.binding_site_range
        try:
            # Ensure that there are at least 3 selected atoms in rec
            assert torch.sum(selected_mask) >= 3
        ...
        lig_rec_squared_dist = torch.sum(
            torch.square(
                lig_position[:, None, :] - rec_position[None, :, :]),
            dim=-1)  # [lig_n_atoms, rec_n_atoms]
        lig_internal_squared_dist = torch.sum(
            torch.square(
                lig_position[:, None, :] - lig_position[None, :, :]),
            dim=-1)  # [lig_n_atoms, lig_n_atoms]
        ...
        lig_atom_type, lig_position, lig_atom_bond_valency, lig_rec_squared_dist = lig_atom_type[
            perm], lig_position[perm], lig_atom_bond_valency[perm], lig_rec_squared_dist[perm]
        lig_con_mat, lig_internal_squared_dist = lig_con_mat[perm][
            :, perm], lig_internal_squared_dist[perm][
            :, perm]

        # Decide the order among lig nodes
        nx_graph = nx.from_numpy_matrix(lig_internal_squared_dist.numpy())
        # return edges starts from the 0-th node (i.e., the contact node here)
        # by default
        edges = list(
            tree.minimum_spanning_edges(
                nx_graph,
                algorithm='prim',
                data=False))
        focus_node_id, target_node_id = zip(*edges)
        node_perm = torch.cat(
            (torch.tensor([0]), torch.tensor(target_node_id)))
        lig_atom_type, lig_position, lig_atom_bond_valency, lig_rec_squared_dist = lig_atom_type[
            node_perm], lig_position[node_perm], lig_atom_bond_valency[node_perm], lig_rec_squared_dist[node_perm]
        lig_con_mat, lig_internal_squared_dist = lig_con_mat[node_perm][
            :, node_perm], lig_internal_squared_dist[node_perm][
            :, node_perm]

        # Prepare training data for sequential generation
        focus_node_id = torch.tensor(focus_node_id)
        # focus_ids denotes the focus atom IDs that are indiced according to
        # the order given by node_perm
        focus_ids = torch.nonzero(
            focus_node_id[:, None] == node_perm[None, :])[:, 1]

        steps_cannot_focus = torch.empty([0, 1], dtype=torch.float)
        idx_offsets = torch.cumsum(torch.arange(lig_n_atoms), dim=0)  # [M]
        # [M-1]
        idx_offsets_brought_by_rec = rec_n_atoms * torch.arange(1, lig_n_atoms)

        for i in range(lig_n_atoms):
            if i == 0:
                # In the 1st step, all we have is the rec. Note that contact
                # classifier should be only applied for the 1st step in which
                # we don't have any lig atoms
                steps_atom_type = rec_atom_type
                steps_rec_mask = torch.ones([rec_n_atoms], dtype=torch.bool)
                # The atom IDs of contact node and the node that are furthest
                # from lig.
                contact_y_or_n = torch.tensor(
                    [rec_contact_id, rec_n_contact_id], dtype=int)
                # The groundtruth for contact_y_or_n
                cannot_contact = torch.tensor([0, 1], dtype=torch.float)
                ...
                dist_to_focus = torch.sum(
                    torch.square(
                        rec_position[rec_contact_id] -
                        rec_position),
                    dim=-
                    1)
                ...
                steps_c1_focus = torch.tensor(
                    [one_step_c1, rec_contact_id],
                    dtype=int).view(
                    1, 2)
                steps_c2_c1_focus = torch.tensor(
                    [one_step_c2, one_step_c1, rec_contact_id],
                    dtype=int).view(
                    1, 3)
                ...
                b = (torch.cross(plane1, plane2) * (focus_pos - c1_pos)
                     ).sum(dim=-1) / torch.norm(focus_pos - c1_pos)
                one_step_torsion = torch.atan2(b, a)
                steps_torsion = one_step_torsion.view(1, 1)

            else:
                one_step_atom_type = torch.cat(
                    (lig_atom_type[:i], rec_atom_type), dim=0)
                steps_atom_type = torch.cat(
                    (steps_atom_type, one_step_atom_type))
                one_step_rec_mask = torch.cat(
                    (torch.zeros(
                        [i], dtype=torch.bool), torch.ones(
                        [rec_n_atoms], dtype=torch.bool)), dim=0)
                steps_rec_mask = torch.cat((steps_rec_mask, one_step_rec_mask))
                one_step_position = torch.cat(
                    (lig_position[:i], rec_position), dim=0)
                steps_position = torch.cat((steps_position, one_step_position))
                steps_batch = torch.cat(
                ...
                steps_cannot_focus = torch.cat(
                    (steps_cannot_focus, (bond_sum == lig_atom_bond_valency[:i, None]).float()))
                ...
                if i == 1:  # c1, c2 must be in rec
                    ...
                    one_step_c1_focus = torch.tensor(
                        [one_step_c1 + idx_offsets[i] +
                         idx_offsets_brought_by_rec[i - 1],
                         focus_id + idx_offsets_brought_by_rec[i - 1]],
                        dtype=int).view(
                        1, 2)
                    steps_c1_focus = torch.cat(
                        (steps_c1_focus, one_step_c1_focus), dim=0)
                    one_step_c2_c1_focus = torch.tensor(
                        ...
                        dtype=int).view(
                        1, 3)
                    steps_c2_c1_focus = torch.cat(
                        (steps_c2_c1_focus, one_step_c2_c1_focus), dim=0)
                    ...
                    steps_angle = torch.cat(
                        (steps_angle, one_step_angle), dim=0)
                    ...
                    steps_torsion = torch.cat(
                        (steps_torsion, one_step_torsion), dim=0)

                else:  # c1, c2 could be in both lig and rec
                    dist_to_focus = torch.cat(
                        (lig_internal_squared_dist[focus_id, : i],
                         lig_rec_squared_dist[focus_id]))
                    _, indices = torch.topk(dist_to_focus, 3, largest=False)
                    one_step_c1, one_step_c2 = indices[1], indices[2]

                    one_step_c1_focus = torch.tensor(
                        [one_step_c1 + idx_offsets[i - 1] +
                         idx_offsets_brought_by_rec[i - 1],
                         focus_id + idx_offsets[i - 1] +
                         idx_offsets_brought_by_rec[i - 1]],
                        dtype=int).view(
                        1, 2)
                    one_step_c2_c1_focus = torch.tensor(
                        [one_step_c2 + idx_offsets[i - 1] +
                         idx_offsets_brought_by_rec[i - 1],
                         one_step_c1 + idx_offsets[i - 1] +
                         idx_offsets_brought_by_rec[i - 1],
                         focus_id + idx_offsets[i - 1] +
...
#                         print(new_pos)
        ...
        steps_focus = torch.cat(
            (steps_focus, focus_ids + idx_offsets[: -1] +
             idx_offsets_brought_by_rec),
            dim=0)
        steps_new_atom_type = lig_atom_type

        # For example, for a rec-lig pair, rec has N atoms and lig has M atoms
        data_batch = {}
        # [N+(1+N)+(2+N)+...+(M-1+N)], which correspond to M steps
        data_batch['atom_type'] = steps_atom_type
        # [N+(1+N)+(2+N)+...+(M-1+N), 3]
