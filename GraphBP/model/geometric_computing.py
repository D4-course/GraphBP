...
def get_nearst_node(pos, batch):
    """
    Function to get nearest node
    """
    j, i = knn_graph(pos, 1, batch)
    adj_nearest = SparseTensor(
        row=i, col=j, value=torch.ones_like(
            j, device=j.device), sparse_sizes=(
            batch.size(0), batch.size(0)))
    j2, i2 = knn_graph(pos, 2, batch)
    adj_nearest2 = SparseTensor(
        row=i2, col=j2, value=torch.ones_like(
            j2, device=j2.device), sparse_sizes=(
            batch.size(0), batch.size(0)))

    return adj_nearest, SparseTensor.from_dense(
        adj_nearest2.to_dense() - adj_nearest.to_dense())


def xyztoda(pos, edge_index, num_nodes):
    """
    Function to get distance and angle from coordinates
    """
    j, i = edge_index  # j->i
...
def xyztodat(pos, edge_index, num_nodes, batch):
    """
    Function to get distance, angle and torsion from coordinates
    """
    j, i = edge_index  # j->i
...
def dattoxyz(f, c1, c2, d, angle, torsion):
    """
    Function to get coordinates from distance, angle and torsion
    """
    c1c2 = c2 - c1
    c1f = f - c1
    c1c3 = c1f * torch.sum(c1c2 * c1f, dim=-1, keepdim=True) / \
        torch.sum(c1f * c1f, dim=-1, keepdim=True)
    c3 = c1c3 + c1

    c3c2 = c2 - c3
    c3c4_1 = c3c2 * torch.cos(torsion[:, :, None])
    c3c4_2 = torch.cross(c3c2, c1f) / torch.norm(c1f, dim=-1,
                                                 keepdim=True) * torch.sin(torsion[:, :, None])
    c3c4 = c3c4_1 + c3c4_2

    new_pos = -c1f / torch.norm(c1f, dim=-1, keepdim=True) * \
        d[:, :, None] * torch.cos(angle[:, :, None])
    new_pos += c3c4 / torch.norm(c3c4, dim=-1, keepdim=True) * \
        d[:, :, None] * torch.sin(angle[:, :, None])
    new_pos += f
