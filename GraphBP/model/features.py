"""
Code to get features for GraphBP
...
def Jn_zeros(n, k):
    """
    Function to calculate zeros of J
    """
...
def spherical_bessel_formulas(n):
    """
    Function for spherical bessel formulas application
    """
    x = sym.symbols('x')
...
def bessel_basis(n, k):
    """
    Function to execute bessel basis
    """
    zeros = Jn_zeros(n, k)
    normalizer = []
...
def sph_harm_prefactor(k, m):
    """
    Function to calculate spherical harm prefactor
    """
...
class Envelope(torch.nn.Module):
    """
    Neural Network class for Envelope
    """
    ...
    def forward(self, x):
        """
        Forward propagation for Envelope
        """
...
class dist_emb(torch.nn.Module):
    """
    Class for neural network to get distance embeddings
    """
    def __init__(self, num_radial, cutoff=5.0, envelope_exponent=5):
        ...
        self.freq = torch.nn.Parameter(
            torch.Tensor(num_radial), requires_grad=False)

        self.reset_parameters()

    def reset_parameters(self):
        """
        Function to reset parameters for distance embedding
        """
        torch.arange(1, self.freq.numel() + 1, out=self.freq).mul_(PI)

    def forward(self, dist):
        """
        Function for forward propagation of distance embeddings neural network
        """
...
class angle_emb(torch.nn.Module):
    """
    Class for neural network to get angle embeddings
    """
    ...
    def forward(self, dist, angle, idx_kj=None):
        """
        Function for forward propagation of angle embeddings neural network
        """
...
class torsion_emb(torch.nn.Module):
    """
    Class for neural network to get torsion embeddings
    """
    def __init__(self, num_spherical, num_radial, cutoff=5.0,
                 envelope_exponent=5):
    ...
    def forward(self, dist, angle, phi, idx_kj):
        """
        Function for forward propagation of torsion embeddings neural network
        """
        dist = dist / self.cutoff
        rbf = torch.stack([f(dist) for f in self.bessel_funcs], dim=1)
